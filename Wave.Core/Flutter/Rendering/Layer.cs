...
using Wave.Core.Flutter.Painting;
using Wave.Core.Flutter.Rendering;

namespace Wave.Core.Flutter.Rendering
{
    class AnnotationEntry<T>(T annotation, Offset localPosition)
    {
        public override string ToString()
        {
            return $"{this.GetType()}(annotation: {annotation}, localPosition: {localPosition})";
        }
    }
...
class TransformLayer extends OffsetLayer
{
    ...
    void addToScene(ui.SceneBuilder builder)
    {
        ...
        {
            _lastEffectiveTransform = Matrix4.translationValues(offset.dx, offset.dy, 0.0)..multiply(_lastEffectiveTransform!);
        }
