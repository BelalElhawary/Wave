...
namespace Wave.Core.Flutter.UI
{
    struct OffsetBase(double dx, double dy)
    {
  public bool IsInfinite => double.IsInfinity(dx) || double.IsInfinity(dy);
  ...
  public static bool operator <(OffsetBase OffsetBase other) => dx < other.dx && dy < other.dy;
